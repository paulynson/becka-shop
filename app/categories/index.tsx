...
const CategoryPage = () => {
  ...
  const catData = useAppSelector((state) => state.store.cat);
  const insets = useSafeAreaInsets();

  // Get the category from the last segment
    ...
    const fetchCategoryProducts = async () => {
      ...
      try {
        const response = await axios.get(
          `https://fakestoreapi.com/products/category/${catData}`
        );
          ...
          headerLeft: () => (
            <>
              ...
              >
                <TouchableOpacity
                  onPress={() => router.back()}
                  className="items-center justify-center w-12 h-12 mr-6 bg-green-200 rounded-full"
                >
